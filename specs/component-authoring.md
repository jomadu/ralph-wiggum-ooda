...
2. Embeds command substitution `$(cat "$VAR")` within the heredoc
3. When heredoc executes, each `$(cat)` command runs and inserts file contents
4. Variables ($OBSERVE, $ORIENT, $DECIDE, $ACT) contain paths to prompt files from configuration
5. Output is valid markdown with clear OODA section headers

**Assembled prompt structure:**

The function outputs a single markdown document with this structure:
...
This assembled prompt is then piped to the AI CLI tool for execution. The clear section headers help the AI understand the OODA framework structure while the component contents provide specific instructions for each phase.
